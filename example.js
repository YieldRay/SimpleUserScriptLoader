...
(function () {
    ...
    let main = {
        ...
        smartIdentify() {
            ...
            if (text !== this.lastText && text !== "") {
                ...
                if (link) {
                    ...
                    util.clog(
                        `文本识别结果：${name} 链接：${link} 密码：${pwd} 耗时：${time}毫秒`
                    );
                    let option = {
                        toast: true,
                    ...
                    Swal.fire(option).then((res) => {
                        ...
                        if (res.isConfirmed || res.dismiss === "timer") {
                            ...
                            let active = util.getValue(
                                "setting_active_in_front"
                            );
                            if (pwd) {
        ...
        parsePwd(text) {
            text = text.replace(/\u200B/g, "");
            let reg =
                /(?<=\s*(密|提取|访问|訪問|key|password|pwd|#)[码碼]?[：:=]?\s*)[A-Za-z0-9]{3,8}/i;
            if (reg.test(text)) {
            ...
            for (let name in opt) {
                ...
                if (panType === name) {
                    if (val.storage === "local") {
                        pwd = util.getValue(val.storagePwdName)
                            ? util.getValue(val.storagePwdName)
                            : "";
                        pwd && this.doFillAction(val.input, val.button, pwd);
                    }
                    if (val.storage === "hash") {
                        if (!/^[A-Za-z0-9]{3,8}$/.test(pwd)) {
                            //过滤掉不正常的Hash
                            return;
                        }
                        pwd && this.doFillAction(val.input, val.button, pwd);
        ...
        doFillAction(inputSelector, buttonSelector, pwd) {
            ...
            let ins = setInterval(async () => {
                maxTime--;
                let input =
                    document.querySelector(inputSelector[0]) ||
                    document.querySelector(inputSelector[1]);
                let button =
                    document.querySelector(buttonSelector[0]) ||
                    document.querySelector(buttonSelector[1]);
        ...
        registerMenuCommand() {
            ...
            GM_registerMenuCommand("⚙️ 设置", () => {
                let html = `<div style="font-size: 1em;">
                              <label class="panai-setting-label">填写密码后自动提交<input type="checkbox" id="S-Auto" ${
                                  util.getValue("setting_auto_click_btn")
                                      ? "checked"
                                      : ""
                              } class="panai-setting-checkbox"></label>
                              <label class="panai-setting-label">前台打开网盘标签页<input type="checkbox" id="S-Active" ${
                                  util.getValue("setting_active_in_front")
                                      ? "checked"
                                      : ""
                              } 
                              class="panai-setting-checkbox"></label>
                              <label class="panai-setting-label">倒计时结束自动打开<input type="checkbox" id="S-Timer-Open" ${
                                  util.getValue("setting_timer_open")
                                      ? "checked"
                                      : ""
                              } class="panai-setting-checkbox"></label>
                              <label class="panai-setting-label" id="Panai-Range-Wrapper" style="${
                                  util.getValue("setting_timer_open")
                                      ? ""
                                      : "display: none"
                              }"><span>倒计时 <span id="Timer-Value">（${
                    util.getValue("setting_timer") / 1000
                }秒）</span></span><input type="range" id="S-Timer" min="0" max="10000" step="500" value="${util.getValue(
                    "setting_timer"
                ...
                document
                    .getElementById("S-Auto")
                    .addEventListener("change", (e) => {
                        util.setValue(
                            "setting_auto_click_btn",
                            e.target.checked
                        );
                    });
                document
                    .getElementById("S-Active")
                    .addEventListener("change", (e) => {
                        util.setValue(
                            "setting_active_in_front",
                            e.target.checked
                ...
                document
                    .getElementById("S-Timer-Open")
                    .addEventListener("change", (e) => {
                        let rangeWrapper = document.getElementById(
                            "Panai-Range-Wrapper"
                        );
                        e.target.checked
